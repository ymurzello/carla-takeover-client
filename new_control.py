            ...
            if event.type == pygame.QUIT:
                ...
                if isinstance(self._control, carla.VehicleControl):
                    if event.key == K_q:
                        ...
                        # world.player.set_autopilot(self._autopilot_enabled)
                        if self._autopilot_enabled:
                            destination = world.agent.vehicle.get_location() + carla.Location(0, 30, 0)
                            print(destination)
                            world.agent.set_destination(world.agent.vehicle.get_location(), destination, clean=True)
                            world.is_agent_controlled = True
                        else:
                            world.is_agent_controlled = False
    ...
    try:
        ...
        while True:
            ...
            if controller.parse_events(client, world, clock):
                return
            if world.is_agent_controlled and not target_reached:
                ...
                if len(world.agent.get_local_planner().waypoints_queue) < 4:
                    print("Target almost reached, mission accomplished...")
                    world.agent.set_destination(world.agent.vehicle.get_location(), world.agent.vehicle.get_location(), clean=True)
                    target_reached = True
                speed_limit = world.player.get_speed_limit()
                world.agent.get_local_planner().set_speed(speed_limit)
                ...
                world.render(display)
                pygame.display.flip()
                if not world.is_agent_controlled and target_reached:
                    target_reached = False
